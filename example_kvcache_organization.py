...
def Vanilla_spec_decnokv22(tokenizer, target, draft, input_ids, gamma=4, max_len=256, top_k=-1, top_p=0.9, temperature=0.6, verbose=False, file_path=None, attention_mask = None): 
    ...
    while n < max_len:
        ...
        # print("small_model_input_full_context: {} attention_mask_for_small_model: {}".format(small_model_input_full_context.shape, attention_mask_for_small_model.shape)) 
        ...
        for _ in range(gamma): 
            if len(generated_ids) > 0: 
                outputs = draft(
                    ...
                    past_key_values = None, 
                    use_cache = False, 
                    attention_mask = torch.cat([attention_mask_for_small_model, torch.ones((1, len(generated_ids))).to(draft.device)], dim = 1), 
                ) 
            else: 
                outputs = draft(
                    input_ids = small_model_input_full_context, 
        ...
        total_accepted_count_per_lsiteration += count # here we accumulate the accepted tokens in each large model iteration step 
    ...
    if verbose:
        # print(f"Use {time2 - time1} sec to generate {n} tokens (now {target_cache.seq_len} tokens), Tokens/s: {n / (time2 - time1)}", flush=True) 
        print(f"accepted rate {acceptance_rate}, avg generated tokens {avg_tokens}") 
        print(f"expected tokens accepted {expected_tokens}") 
        ...
        for i in range(iterationscounts): # we need a forloop 
            ...
            for batch in tqdm(dataloader): 
                ...
                if args.use_small_draft: 
                    ...
                    ''' 
                    acceptancer, draftcount, total_accepted_tokens, num_verifications, expected_tokens = Vanilla_spec_decnokv22(tokenizer, 
                                                target_largemodel, 
                                                small_model, 
                                                input_ids, 
