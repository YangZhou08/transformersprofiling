...
from ...generation.utils import GreedySearchEncoderDecoderOutput, GreedySearchDecoderOnlyOutput 
...
class LlamaRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, seq_len=None):
        ...
        print("seq_len {} max_seq_len_cached {}".format(seq_len, self.max_seq_len_cached)) 
...
class LlamaForCausalLM2(LlamaPreTrainedModel):
    ...
    def resetcount(self): 
        ...
        for layer in self.model.layers: 
            layer.mlp.resetgenerationiterateingcount() 
    ...
    def prepare_inputs_for_generation(
        ...
        if self.generateiterationcount % (self.chunksize + 1) == 0 and past_key_values is not None: 
            ...
            for layer in past_key_values: 
                new_layer = [] 
                for kv in layer: 
                    new_layer.append(kv[:, :, : -self.chunksize, :]) 
...
class LlamaWeirdLargeIterative(LlamaPreTrainedModel): 
    """ 
    We call this autoregressive Medusa model 
    """ 
    # almost identical to LlamaWeirdLarge3, but weird fix for some model 
    ...
    def forward(
        ...
        for effective_kernel_size in range(1, self.sliding_window_length + 1): 
            ...
            addonmodeloutput = self.addonsmallmodel( 
                ...
                return_dict = True, 
                # start_idx = self.addonmodel_start, # NOTE this is very important 
                start_idx = effective_kernel_size + 1, # NOTE this is very important 
                eval_mode = False, 
            ...
            if label_adjustment: # we adjust the labels to be completely information loss free 
                ...
                for i in range(labels_addition.shape[1]): 
                    ...
                    if old_label_count < labels.shape[1]: 
                        ...
                        newlabels = torch.cat([newlabels, labels[:, old_label_count : min(old_label_count + effective_kernel_size, labels.shape[1])]], dim = 1) 
                    # old_label_count += self.sliding_window_length 
                    old_label_count += effective_kernel_size 
...
class LlamaWeirdLargeFullCoverage(LlamaPreTrainedModel): 
    """ 
    When finetuning the large model, we try to use all the output of the decoder layers 
    """ 
    # almost identical to LlamaWeirdLarge3, but weird fix for some model 
    _tied_weights_keys = ["lm_head.weight"] 
    ...
    def forward(
        ...
        for j in range(self.sliding_window_length): 
            ...
            if nummagic != 0: 
                ...
                newsmallinputids = large_input_ids 
                newattentionmask = original_attention_mask 
            
            labels = newsmallinputids.clone() 
            ...
            # print("small_input_ids: {}".format(small_input_ids[0])) 
            print("small_input_ids: {}".format(newsmallinputids[0])) 
            print("self.addonmodel_start {}".format(self.addonmodel_start)) 
            print("sliding_window_length {}".format(self.sliding_window_length)) 
            ...
            addonmodeloutput = self.addonsmallmodel( 
                input_ids = newsmallinputids, 
                attention_mask = newattentionmask, 
                past_key_values = None, 
                condensed_embeds = selected_hidden, 
                labels = None, 
                output_attentions = True, 
                output_hidden_states = None, 
                return_dict = True, 
...
class LlamaWeirdLargeTest(LlamaPreTrainedModel): 
    ...
    def __init__(self, config): 
        ...
        self.lm_head = nn.Linear(self.config.hidden_size, self.config.vocab_size, bias = False) 
        small_config = LlamaConfig.from_pretrained("Cheng98/llama-160m") 
        print("self.config.hidden_size {}".format(self.config.hidden_size)) 
        self.addonsmallmodel = SimpleSmallModel(small_config, target_model_dim = self.config.hidden_size) # sliding_window_length is set elsewhere 
        self.small_model_dtype = torch.bfloat16 
        self.use_mse_loss = False 
        self.ce_loss_only = False 
        ...
        self.use_cosinesimilarity = False 
        self.generate_iteration_count = 0 
        self.generate_model_hidden_states = None # this field saves the intermediate tensors generated by the large model 
        self.generate_model_past_key_values = None 
        self.tokenizer_bos_id = 1 
        self.tokenizer_pad_id = 2 
    ...
    def forward(
        ...
        if (large_input_ids.shape[1] - 1) % self.sliding_window_length != 0: 
            missinglength = self.sliding_window_length - ((large_input_ids.shape[1] - 1) % self.sliding_window_length) 
            large_input_ids = torch.cat([torch.full((large_input_ids.shape[0], missinglength), self.tokenizer_pad_id, dtype = large_input_ids.dtype).to(large_input_ids.device), large_input_ids], dim = 1) 
            attention_mask = torch.cat([torch.zeros((attention_mask.shape[0], missinglength), dtype = attention_mask.dtype).to(attention_mask.device), attention_mask], dim = 1) 
    ...
    def forward_generate(
        ...
        return CausalLMOutputWithPastLargeDistance3(
            loss = loss, 
            logits = logits, 
            past_key_values = self.generate_model_past_key_values, 
            hidden_states = None, 
            attentions = None, 
            l2_distance = None, 
            ce_loss = None, 
...
class LlamaWeirdLargeRecoveringModeOn(LlamaPreTrainedModel): 
    ...
    def forward_generate(
        ...
        for i in range(self.sliding_window_length): 
            # NOTE for this case, we use the pass-in attention mask 
            # print(colored("running the large model side", "green")) 
            # print("shape of large_input_ids {}".format(large_input_ids.shape)) 
