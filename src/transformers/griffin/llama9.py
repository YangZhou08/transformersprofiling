...
def get_llama_griffin(model,  k_schedule, notcats = False): 
    config = model.config
    for i, l in enumerate(model.model.layers):
        if not notcats: # if cats 
...
class GriffinLlamaMLP2(nn.Module): 
    def __init__(self, config, k_factor):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        self, input_ids, full_past_key_values=None, sparse_past_key_values = None, attention_mask=None, inputs_embeds=None, **kwargs
    ): 
        if full_past_key_values is not None:
            past_length = full_past_key_values[0][0].shape[2]
            
            if sparse_past_key_values is None: 
                # this happens at the second iteration 
                sparse_past_key_values = [] 
                for i in range(len(full_past_key_values)): 
                    new_layer = [] 
                    for j in range(len(full_past_key_values[i])): 
                        new_kv = full_past_key_values[i][j].clone() 
                        new_layer.append(new_kv) 
                    sparse_past_key_values.append(tuple(new_layer)) 
                sparse_past_key_values = tuple(sparse_past_key_values) 
            else: 
                assert past_length == sparse_past_key_values[0][0].shape[2], "The past length should be the same" 
        ...
        if attention_mask is not None and position_ids is None:
            ...
            if full_past_key_values:
                position_ids = position_ids[:, -input_ids.shape[1] :]

        # if `inputs_embeds` are passed, we only want to use them in the 1st generation step
        if inputs_embeds is not None and full_past_key_values is None:
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        sequence_token_corrected = 0 
        sequence_token_generated = 0 
        ...
        while True:
            ...
            if gen_len > 0:
                ...
                outputs = self(
                    ...
                    output_hidden_states=output_hidden_states,
                    use_cache = True, 
                    past_key_values = model_inputs["sparse_past_key_values"], 
                ) 
                ...
                outputs = self(
                    ...
                    position_ids = model_inputs["position_ids"], 
                    use_cache = True, 
                    past_key_values = model_inputs["full_past_key_values"], 
                    return_dict=True,
                    output_attentions=output_attentions,
                    output_hidden_states=output_hidden_states,
                ) 
            ...
            if gen_len > 0: # small model runs once, large model runs once 
                past_key_values = model_inputs["full_past_key_values"] 
                self.set_mlp_inference_mode("full")
                outputs = self(
                    input_ids=model_inputs["input_ids"], 
                ...
                next_token_logits = outputs.logits[:, -1, :] 
                next_tokens_scores = logits_processor(input_ids, next_token_logits)
                next_tokens_proba = torch.softmax(next_tokens_scores, dim = -1) 
                sparsetokenproba = next_tokens_proba[0][next_tokens] 
                assert self.cothr is not None 
                if sparsetokenproba < self.cothr: 
                    sequence_token_corrected += 1 
                    next_tokens = torch.argmax(next_tokens_scores, dim = -1) 
                self.num_steps += 1
        ...
        self.total_tokens_corrected += sequence_token_corrected 
        self.total_tokens_generated += sequence_token_generated 
        print("generate tokens {} corrected tokens {} global_generate {} global_corrected {} ratio {} global_ratio{}".format(sequence_token_generated, sequence_token_corrected, self.total_tokens_generated, self.total_tokens_corrected, sequence_token_corrected/sequence_token_generated, self.total_tokens_corrected/self.total_tokens_generated)) 
