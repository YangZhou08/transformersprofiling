...
parser.add_argument("--requeuepassword", type = str, default = None) 
...
class CustomTrainer(Trainer): 
    ...
    def compute_loss(self, model, inputs, return_outputs = False, evaluation_mode = True): 
        ...
        if isinstance(getattr(model, "module", model), SimpleSmallModel) or isinstance(model, SimpleSmallModel) == True: 
            ...
            print("the attention mask shape is {}".format(attention_mask.shape)) 
            print("the label2 shape is {}".format(label2.shape)) 
            addedon_length = 0 
            outputs = model(
                input_ids = input_ids, 
                attention_mask = attention_mask, 
...
class CustomDataset: 
    def __init__(self, data_dir, tokenizer = None, max_length = 256, kernel_size = 7, topk = None, prompt_length = 64, use_c4 = False): 
            ...
            if kernel_size != 4: 
                if hostname == "ada": 
                    ...
                    if model_name == "llama2_7b": 
                        filename = "c4synthesized_file1_{}_0.json".format(1) 
...
if args.usedatasettype == "synthesized": 
    ...
    datasetnew = CustomDataset(max_length = 260, data_dir = datapath_c4, tokenizer = tokenizer, kernel_size = kernel_size, use_c4 = True) 
...
if args.requeuepassword is None: 
    model_path = dir_models + "{}_{}_{}_{}_{}/".format(model_name, args.experiment_setting, args.kernel_size, commit_hash, hash_of_time) 
else: 
    model_path = dir_models + "{}_{}_{}_{}/".format(model_name, args.kernel_size, args.requeuepassword, args.experiment_setting) 
...
training_args = TrainingArguments(
    ...
    per_device_train_batch_size = 112,  # the training batch size, put it as high as your GPU memory fits
    gradient_accumulation_steps=4,  # accumulating the gradients before updating the weights
    per_device_eval_batch_size=112,  # evaluation batch size
    # logging_steps=1, 
    logging_steps = 100 if not args.debug else 5,             # evaluate, log and save model checkpoints every 1000 step
    # save_steps=1000, 
    # save_steps = 2000, 
    save_steps = 100 if not args.debug else 5, 
    # learning_rate=5e-7, 
    # learning_rate=5e-5, 
    learning_rate=2e-4, 
    # learning_rate = 1e-4, 
...
def check_folder_exists(folder_path): # this function is for checking whether previous checkpoint exists 
...
def find_latest_checkpoint(checkpoint_dir):
    ...
    # Filter out items that are not directories or don't follow the naming convention
    checkpoint_folders = [item for item in all_items if os.path.isdir(os.path.join(checkpoint_dir, item)) and item.startswith("checkpoint-")] # both a directory and also starts with "checkpoint" prefix 
    # Extract the numerical part of the checkpoint names and sort them
    latest_checkpoint = max(checkpoint_folders, key=lambda x: int(x.split('-')[1]), default=None) # max takes in a key argument that is basically a function 
...
if args.requeuepassword is not None: 
    if does_checkpoint_folder_exist(model_path) != True: # the folder is always there but it might be empty with no checkpoint folders actually 
        ...
        # we found previous instances of checkpoint 
        print(colored("Found previous instances of checkpoint", "green")) 
        latest_checkpoint = find_latest_checkpoint(model_path) 
        resumecheckpointpath = latest_checkpoint 
else: 
...
if trainer.accelerator.is_main_process and has_wandb: 
    ...
    wandb.init(project = "llm160m", 
               ...
               id = args.requeuepassword, 
               resume = True if resumecheckpointpath is not None else False, 
    ) 
